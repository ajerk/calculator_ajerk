 ...
 //数字按键  
    function number(n){           
                    if(apoint)                                             //num[step]有小数点
                        ...
                        if(num[step] == null)
                            num[step]=(parseInt(num[step]+n)).toString();
                        else if(parseFloat(num[step]) == 0)
                            num[step]=(parseInt(num[step].toString()+n)).toString();     //“00001”或者"000002"的数字
                        else 
                            num[step]=(num[step].toString()+n).toString();     
                        }
                    if(step == 0)                                           
...
//小数点按键
    function point(){
            if(!apoint){                                                      //该数字还没有小数点时才能加小数点
                ...
                else if( num[step] == 0)                                                     //num[step] 为 0 或 null 
                    num[step] = 0 + ".";
                else if(step == 0)
                    num[step] = 0 + ".";
                else 
...
//等于号按键   
    function equal(){ 
        if(!equa){
            stop = 1;
            for(i = 0 ; i < step ; i++){
                if(operation[step-i-1] == "÷"){                     //  "÷"号检查
                    if(parseFloat(num[step-i-1+1]) == 0)
                        stop = 2;
                }
            }
            for(i = 0 ; i < (num[0].toString()).length ; i++){     //处理当num【0】的绝对值值很大、用"**e***"的科学计数法表示时可能发生的异常
                if((num[0].toString()).substring(i,i+1) == "e"){             
                    if((num[0].toString()).length == i+1){        //  "**e"的情况
                        stop = 3;
                    }
                    else                                         //  "**e**.***"的情况
                        for(j = i+1 ; j < (num[0].toString()).length ; j++){
                            if((num[0].toString()).substring(j,j+1) == "." ){
                                stop = 3;
                            }
                        }
                }
            ...
            if(stop==1){
                ...
                for(i = 0 ; i < step+1 ; i++){                                  //将num[i]值为".***"的改为"0.***"
                if(num[i] !== null){
                    if((num[i].toString()).substring(0,1) == ".")
                        num[i] = "0" + num[i];
                    }
                }          
                ...
                if(num[0] !== null)
                {
                    ...
                    for(i = 0 ; i < (result.toString()).length ; i++)               // 科学计数法
                        if((result.toString()).substring(i,i+1) == "+"){              
                            result = (result.toString()).substring(0,i) + (result.toString()).substring( i+1 , (result.toString()).length);
                        }
